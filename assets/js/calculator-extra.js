...
function updateGlowwire() {
  ...
  var map = {
    support: { t: "650°C", std: "IEC 60335-1 第 30.2 节 / GB 4706.1：保持带电部件就位的非金属支撑件——基础档", rel: "配套：球压（支撑件不变形）＋ 必要时针焰", tip: "变压器骨架、继电器底座、端子排支架等" },
    terminal: { t: "650°C", std: "载流连接点 / 端子附近绝缘——按 30.2 基础档", rel: "配套：球压 + 耐热（材料 RTI）", tip: "连接点接触不良是常见起火源，选耐热阻燃材料" },
    pcb: { t: "550–650°C", std: "PCB 基材：按是否支撑带电件定档（支撑件 650，一般基材 550）", rel: "配套：针焰（基材点着后不蔓延）", tip: "FR-4 常规可通过 550–650°C；无卤牌号注意与 CTI 一起核" },
    enclosure: { t: "750°C", std: "外壳 / 通风口（可能被内部引燃且是火焰传播路径）——常见 750°C", rel: "配套：球压（外壳高温不变形）＋ 针焰", tip: "外壳不一定是最高档——内部故障引燃外壳后，外壳要阻止火焰蔓延到外部" },
    wire: { t: "550°C", std: "电源线固定件 / 内部线材固定——低风险档（550°C）", rel: "配套：线材 VW-1 / FT1 或灼热丝", tip: "线材固定件松动会导致机械应力，但火灾风险低于连接点" },
    unattended: { t: "850°C", std: "无人看管器具（30.2.2 附加）：高温区 / 可能被点燃并蔓延——850°C 档", rel: "配套：针焰（蔓延评估）＋ 阻燃外壳", tip: "电热器具、电动工具等无人看管场景要求更高；确认产品标准是否含 30.2.2" }
  };
  var m = map[pos];
  if (!m) return;
  $("gwResult").textContent = m.t;
  $("gwNote").innerHTML = "<b>" + m.std + "</b><br>相关试验：" + m.rel + "<br>提示：" + m.tip + "<br><small>GWFI/GWIT 是材料等级（最高通过档/起燃温度），档位数字是本次试验温度——两者不同；正式档位以产品标准防火章节为准。</small>";
}
