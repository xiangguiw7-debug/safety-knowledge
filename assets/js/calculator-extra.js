...
function updateGlowwire() {
  var pos = $("gwPos").value;
  // 依据：IEC 60335-1:2016 / GB/T 4706.1-2024 第 30.2 条（2024-07-24 发布、2026-08-01 实施，代替 GB 4706.1-2005）。
  // 新版把"保持带电部件就位"从旧版 30.2.1(650°C) 降为 30.2.1 基础档 550°C，并把载流连接件支撑单独分级：
  //   30.2.1    一般非金属部件基础档 = 550°C（GWFI≥550 或 HB40 或 HBF 可豁免）
  //   30.2.2    有人照管器具：支撑载流连接件及 3mm 内 → 载流>0.5A 用 750°C；其他 650°C
  //   30.2.3.1  无人照管器具：支撑载流>0.2A 连接件及 3mm 内（小部件除外）→ 850°C（GWFI≥850 豁免）
  //   30.2.3.2  无人照管器具：支撑连接件及 3mm 内 → 载流>0.2A 用 750°C；其他 650°C（GWIT/GWFI 可豁免）
  //   30.2.4    印刷电路板基材 → 附录 E 针焰试验（不是灼热丝档位）
  var map = {
    support: { t: "550°C", std: "IEC 60335-1 第 30.2.1 节：一般非金属部件基础档 → GWT 550°C（保持带电部件就位的支撑件也走此档；GWFI≥550 或 HB40 或 HBF 可豁免）", rel: "配套：30.1 球压（支撑带电部件 125°C / 外部部件 75°C）", tip: "旧版 GB 4706.1-2005 曾要求 650°C，新版已统一为基础档 550°C" },
    terminal: { t: "750°C", std: "载流连接点 / 端子支撑：有人照管器具且连接件正常载流 >0.5A → 30.2.2 档 750°C；载流 ≤0.5A 的连接 → 650°C", rel: "配套：球压 + 材料 GWFI/GWIT", tip: "端子不是固定 650°C——先判看管条件与载流量" },
    unattend: { t: "850°C", std: "无人照管器具：支撑载流 >0.2A 连接件及 3mm 内 → 30.2.3.1 档 850°C（GWFI≥850 豁免）；其他连接支撑 → 30.2.3.2 档 750°C（>0.2A）/650°C（≤0.2A）", rel: "配套：针焰（火焰持续>2s 时对连接区圆柱体内部件追加）", tip: "电热器具、电动工具等无人看管场景取最高档" },
    pcb: { t: "针焰（非灼热丝）", std: "PCB 基材 → 30.2.4：附录 E 针焰试验（30s±1s，基材余焰 ≤15s），火焰施加于散热最差边缘", rel: "配套：V-0/VTM-0 材料可豁免", tip: "PCB 在新版标准里不走灼热丝档位，走针焰" },
    enclosure: { t: "550°C 基础档", std: "外壳 / 通风口等一般非金属部件 → 30.2.1（550°C）；若外壳内表面距载流连接件 3mm 内，则按 30.2.2/30.2.3 的连接档位考核", rel: "配套：30.1 球压（外部部件 75°C）+ 针焰（蔓延路径）", tip: "外壳本身不是固定 750/850°C——看是否支撑连接件及距离" },
    wire: { t: "550°C 基础档", std: "电源线固定件 / 内部线材 → 30.2.1 基础档（550°C）；注意：试验不在电线绝缘上进行（30.2 注）", rel: "配套：线材 VW-1 / FT1 按产品标准", tip: "固定件机械强度与火灾风险分开评估" }
  };
  var m = map[pos];
  if (!m) return;
  $("gwResult").textContent = m.t;
  $("gwNote").innerHTML = "<b>" + m.std + "</b><br>相关试验：" + m.rel + "<br>提示：" + m.tip + "<br><small>版本说明：本档位按 IEC 60335-1:2016 / GB/T 4706.1-2024 第 30.2 条（代替 GB 4706.1-2005）。旧版 30.2.1 的 650°C（保持带电部件就位/附加绝缘）与 30.2.2 的 850°C（基本绝缘）已重构为：30.2.1 基础 550°C + 载流连接件分档（有人照管 750/650°C；无人照管 850/750/650°C）+ PCB 针焰。550/750°C 仍是 IEC 60695/62368 体系常用档；GWFI/GWIT 是材料等级（最高通过档/起燃温度），与试验温度不同——正式档位以产品标准防火章节为准。</small>";
}

// ===== 针焰 / 球压速查 =====
function updateNeedleFlame() {
  var map = {
    conn: { t: "30s±1s", note: "IEC 60335-1 附录 E：载流连接件附近（火焰持续>2s 时对连接区上方直径20mm×高50mm圆柱体内部件追加针焰）" },
    pcb: { t: "30s±1s", note: "IEC 60335-1 30.2.4：PCB 基材针焰，火焰施加于散热最差边缘；基材燃烧持续时间 ≤15s（一般部件 ≤30s）" },
    general: { t: "30s±1s", note: "IEC 60335-1 附录 E 通用：施加火焰 30s±1s；距拐角至少 10mm 施加；余焰 tb ≤30s（PCB ≤15s）" }
  };
...
function updateBallPressure() {
  var map = {
    external: { t: "75°C±2°C", note: "外部部件（30.1）：烘箱温度 40°C+第11章最大温升，但至少 75°C±2°C" },
    support: { t: "125°C±2°C", note: "支撑带电部件（含连接）的部件（30.1）：至少 125°C±2°C" },
    suppinsul: { t: "25°C+第19章温升", note: "附加绝缘/加强绝缘热塑材料（30.1）：25°C±2°C 加第19章试验确定的最高温升（若更高）" }
  };
...
function updateFireDecision() {
  var map = {
    appliance: { combo: "灼热丝 550–850°C（30.2 分档：基础 550 / 连接件有人照管 650–750 / 无人照管 850）· 针焰 30s（附录E/PCB）· 球压 75–125°C · UL94 V-0", basis: "IEC 60335-1 / GB/T 4706.1-2024 第 30 章（耐热耐燃）" },
...
var IP_SOLID = {
  ...
  2: { n: "直径 ≥ 12.5mm", d: "铰接试指 10N（+12.5mm 球 30N），手指不能进入" },
