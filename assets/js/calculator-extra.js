...
function updateGlowwire() {
  var pos = $("gwPos").value;
  // 档位体系（教学参考，正式按产品标准防火章节）：U = 额定工作电压；档位数字 = 灼热丝试验温度
  var map = {
    // 档位体系说明：IEC 60335-1 / GB 4706.1 第 30 章核心档位为 650°C（30.2.1 基础）与 850°C（30.2.2 无人看管附加）；550/750°C 是 IEC 60695 / 62368 语境常用档，正式档位以产品标准防火章节为准
    support: { t: "650°C", std: "IEC 60335-1 第 30.2.1 节：保持带电部件就位的非金属支撑件（变压器骨架/继电器底座/端子支架）→ GWT 650°C，所有器具的基础要求", rel: "配套：球压（支撑件高温不变形）", tip: "变压器骨架、继电器底座、端子排支架等" },
    terminal: { t: "650°C", std: "载流连接点 / 端子附近绝缘 → 30.2.1 基础档（接触不良是常见起火源）", rel: "配套：球压 + 材料 RTI 耐温", tip: "连接点选阻燃耐热材料，避免接触不良" },
    pcb: { t: "650°C 或针焰", std: "PCB 基材支撑带电走线 → 30.2.1（650°C）；部分标准允许针焰替代（30.2.3/30.2.4）", rel: "配套：针焰（基材点着后不蔓延）", tip: "FR-4 常规可通过；无卤与 CTI 一起核" },
    enclosure: { t: "650–850°C 视看管条件", std: "外壳 / 通风口：有人看管器具 → 30.2.1（650°C）；无人看管且可能被点燃蔓延 → 30.2.2（850°C 或针焰）", rel: "配套：球压 + 针焰", tip: "外壳不是固定 750°C——先判定器具是否无人看管" },
    wire: { t: "650°C", std: "电源线固定件 / 内部线材 → 30.2.1 基础档（650°C）", rel: "配套：线材 VW-1 / FT1", tip: "固定件机械强度与火灾风险分开评估" },
    unattended: { t: "850°C", std: "无人看管器具（30.2.2）：可能被故障点燃并蔓延的部件 → GWT 850°C 或针焰（最高要求档）", rel: "配套：针焰（蔓延评估）＋ 阻燃外壳", tip: "电热器具、电动工具等无人看管场景" }
  };
  var m = map[pos];
  if (!m) return;
  $("gwResult").textContent = m.t;
  $("gwNote").innerHTML = "<b>" + m.std + "</b><br>相关试验：" + m.rel + "<br>提示：" + m.tip + "<br><small>550°C / 750°C 是 IEC 60695 / 62368 体系常用档，家电（60335/GB 4706.1）核心档为 650°C / 850°C；GWFI/GWIT 是材料等级（最高通过档/起燃温度），与试验温度不同——正式档位以产品标准防火章节为准。</small>";
}
