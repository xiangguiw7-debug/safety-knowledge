...
(function () {
  ...
  var PD_INFO = {
    1: {
      ...
      action: "可按最清洁条件取值，但先确认产品标准是否提供这一档：灯具 GB 7000.1 第 11 章的两张距离表都以污染等级 2 为基准（表11.1 / 附录 U 表 U.1），没有 PD1 列；医疗 GB 9706.1 表11 / 表12 按污染等级 2 给出，能否按 PD1 放宽要看标准相应条款（本工具仍按 PD2，偏保守）；通用 IEC 60664-1 表另有 PD1 列，数值更小。"
    },
    2: {
      name: "PD2",
    ...
    3: {
      ...
      action: "灯具：GB 7000.1 第 11 章没有 PD3 表格——应改用 IEC 60664-1 的 PD3 列（通用模式），或先用密封 / 涂覆 / 灌封把绝缘处微环境降到 PD2。医疗：标准表按污染等级 2 给出，需先用外壳 / 密封把微观环境降到 2 级，或按标准特殊条款评估。通用：按 PD3 列查表（数值最大）。"
    },
    4: {
      name: "PD4",
      title: "污染等级 4 · 持久导电",
  ...
  function applyToSpacing(kind) {
    ...
    if (typeof currentState === "function" && typeof applyState === "function") {
      ...
      if (kind === "medical") {
        ...
        // 污染等级只在 IEC 60664-1 依据下才有意义：带入 PD 并自动切到该依据
        st.std = "luminaire";
        st.lumBasis = "iec60664";
        st.pd = String(Math.min(pd, 4));
      }
      applyState(st);
    }
    if (pd >= 4) {
      note("⚠ 判定为 PD4：已带入间距工具（依据 IEC 60664-1）。但本工具只收录了 PD1 / PD2 / PD3 列，PD4 必须按标准原表取值，或先用外壳密封 / 灌封 / 排水 / 除湿把绝缘处的污染降下来。");
    } else if (kind === "medical" && pd >= 3) {
      note("⚠ 判定为 PD" + pd + "：医疗表（表11 / 表12）按污染等级 2 给出——需先用外壳 / 密封把绝缘处的微观环境降到 PD2，或按标准特殊条款评估。");
    } else {
      note("已带入间距工具：依据 = " + (kind === "medical" ? "医疗 · GB 9706.1 表11 / 表12（污染等级 2）" : "IEC 60664-1 绝缘配合 · 污染等级 PD" + Math.min(pd, 4) + "（爬电按该列，间隙按额定冲击电压）") + "。");
    }
  }
